...
bool new_fixed_size_stream(
    ...
    //the cycle index must be < the cycle index of consumer position!
...
bool fixed_size_stream_try_claim(
    ...
    if (producer_position >= claim_limit) {
        ...
        claim_limit = consumer_position + stream->max_gain;
        if (producer_position < claim_limit) {
            //update the cached consumer position for other producers
    ...
    if (cycle_position < stream->cycle_length) {
        ...
        *claimed_message = (stream->buffer) +
                           ((active_cycle_index * (stream->cycle_length * stream->aligned_message_size)) +
                            (cycle_position * stream->aligned_message_size) + MESSAGE_STATE_SIZE);
        return true;
...
void fixed_size_stream_commit_claim(const uint8_t *const claimed_message_address) {
...
uint32_t fixed_size_stream_read(
        const struct fixed_size_stream_t *const stream,
...
uint64_t load_producer_position(const struct fixed_size_stream_t *const stream) {
    const uint32_t active_cycle_index = atomic_load_explicit(stream->active_cycle_index, memory_order_acquire);
    _Atomic uint64_t *producers_active_cycle_claim_address =
            stream->producers_cycle_claim + (sizeof(uint64_t) * active_cycle_index);
    const uint64_t producer_claim = atomic_load_explicit(producers_active_cycle_claim_address, memory_order_relaxed);
...
uint64_t load_consumer_position(const struct fixed_size_stream_t *const stream) {
    const uint64_t consumer_position = atomic_load_explicit(stream->consumer_position, memory_order_relaxed);
    return consumer_position;
}

uint32_t fixed_size_stream_size(const struct fixed_size_stream_t *const stream) {
